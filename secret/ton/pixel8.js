  //----------------------------------//
 //--2024 All rights reserved Nxrix--//
//----------------------------------//
...
pixel8.init = async (canvas, width = 128, height = 128) => {
  ...
  window.strtoarr = (str) => {
    const numberArray = str.split("").map(char => {
      const number = char.charCodeAt();
      return number;
    });
    return numberArray;
  }
  ...
  window.arrtostr = (arr) => {
    const stringArray = arr.map(number => {
  ...
  window.strtouni = (str) => {
    return ats(strtoarr(str));
  }

  window.unitostr = (uni) => {
    return arrtostr(sta(uni)).replaceAll("\x00","");
  }

  window.arruni = (arr) => {
    const uniarr = arr.map(char => {
      return strtouni(char);
    });
    return uniarr;
  }
  ...
  window.btot = (binary) => {
    let text = "";
    binary.match(/.{1,8}/g).map(function(byte) {
